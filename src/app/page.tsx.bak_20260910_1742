...
import { Search, TrendingUp, Sparkles, Filter, Loader2, Flame, Settings, Target, BarChart3, Info, BookOpen, X, HelpCircle, AlertTriangle, History, Trash2, Calendar, FlaskConical, CheckCircle2, XCircle, LogOut, RefreshCw, Download, ChevronLeft, ChevronRight, CheckSquare, Square, Compass, Zap, Clock, Activity } from 'lucide-react';
...
export default function DashboardPage() {
  ...
  const [showMockTrading, setShowMockTrading] = useState(false); // 新增模擬交易狀態
  ...
  // ── 本地數據倉庫與下載最新股票數據 ──
  const [syncStatus, setSyncStatus] = useState<SyncStatus>({ hasData: false, lastSyncTime: null, totalStocks: 0 });
  ...
  const handleDownloadLatestData = async () => {
    setIsSyncing(true);
    setSyncMessage('正在連線證交所與上櫃中心...');

    const res = await DataSyncService.downloadLatestMarketData((progress) => {
      setSyncMessage(progress.stage);
    });

    if (res.success) {
      const updatedStatus = DataSyncService.getStatus();
      setSyncStatus(updatedStatus);
      alert(`✅ 成功下載最新股票數據！\n總共載入 ${res.count} 档股票。\n最新數據時間：${res.timestamp}`);
    } else {
      alert(`❌ 數據下載失敗：${res.error || '未知錯誤'}`);
    }
  ...
  const runSingleStockAnalysis = async (stockId: string, stockName?: string) => {
    ...
    try {
      ...
      if (!batchRes.ok) {
        ...
        throw new Error(`分析請求失敗 (HTTP ${batchRes.status}): ${text.slice(0, 100)}`);
      }
  ...
  const runScan = async (overrideTerm?: string, forceRefresh?: boolean) => {
    ...
    try {
      // Phase 1: Directed Fetch (優先從本地預載數據庫獲取)
      setProgress({ current: 0, total: 0, phase: `正在獲取 ${MARKET_NAMES[market]} - ${currentSectorName} 數據...` });

      let snapshot: StockData[] = forceRefresh ? [] : DataSyncService.getLocalSnapshot(market);

      if (snapshot.length === 0 || forceRefresh) {
  ...
  const runEnhancedScan = async () => {
    ...
    try {
      let snap: StockData[] = DataSyncService.getLocalSnapshot(market);
      if (snap.length === 0) {
        const snapshotRes = await fetch(`/api/market/snapshot?market=${market}&sector=${sector}`);
        const snapshotJson = await snapshotRes.json();
        if (!snapshotJson.success) throw new Error(snapshotJson.error);
        snap = snapshotJson.data;
      }
      const t1 = Date.now();
  ...
  return (
    <AuthGuard>
      ...
      <header className="mb-14 text-center">
        ...
        <button
          onClick={() => setShowMockTrading(true)}
          className="inline-flex items-center gap-2 px-5 py-2 rounded-xl bg-white/5 border border-white/10 hover:bg-white/10 hover:border-teal-500/50 transition-all text-teal-400 font-black mb-10 ml-4 group"
        >
          <Activity className="w-5 h-5 group-hover:scale-110 transition-transform" />
          模擬交易
        </button>

        {/* 🔄 下載最新股票數據按鈕暨狀態時間 (Header 顯眼區) */}
        <div className="inline-block mb-10 ml-4 align-top">
          <button
            onClick={handleDownloadLatestData}
            disabled={isSyncing}
            className="inline-flex items-center gap-2 px-5 py-2 rounded-xl bg-cyan-500/20 border-2 border-cyan-500/50 hover:bg-cyan-500/30 hover:border-cyan-400 transition-all text-cyan-300 font-black group shadow-lg shadow-cyan-500/20 disabled:opacity-50"
          >
            <RefreshCw className={`w-5 h-5 group-hover:rotate-180 transition-transform ${isSyncing ? 'animate-spin' : ''}`} />
            {isSyncing ? (syncMessage || '正在下載數據...') : '下載最新股票數據'}
          </button>
          <div className="text-xs text-cyan-300/90 mt-1.5 font-mono text-center font-bold tracking-tight">
            {syncStatus.lastSyncTime ? `📅 最新數據時間：${syncStatus.lastSyncTime}` : '⚡ 尚未預載內部數據'}
          </div>
        </div>
