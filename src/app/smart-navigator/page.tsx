...
export default function SmartNavigatorPage() {
    ...
    const [failedStocks, setFailedStocks] = useState<Array<{id: string, name: string}>>([]);
    ...
    const handleAutoFilter = async (stocksToRetry?: Array<{id: string, name: string}>) => {
        ...
        // If not retrying, reset everything. If retrying, we append to existing results.
        if (!stocksToRetry) {
            setFilterResults([]);
            setFailedStocks([]);
        }
        ...
        try {
            let uniqueStocks: Array<{id: string, name: string}> = [];

            if (stocksToRetry) {
                uniqueStocks = stocksToRetry;
            } else {
                ...
                uniqueStocks = Array.from(stockMap.entries()).map(([id, name]) => ({ id, name }));
            }
            ...
            const BATCH_SIZE = 5;
            const newValidResults: any[] = [];
            const currentlyFailed: Array<{id: string, name: string}> = [];
            ...
            for (let i = 0; i < uniqueStocks.length; i += BATCH_SIZE) {
                    ...
                    try {
                        ...
                        if (json.success && json.data) {
                            if (filterMode === 'green') {
                                ...
                                return { skip: true }; // Marker for successful fetch but doesn't meet criteria
                            } else {
                ...
                newValidResults.push(...resolved.filter(r => r !== null && !r.skip));
            }

            // Update failed stocks state
            if (stocksToRetry) {
                // If we were retrying, filter out the ones that succeeded this time
                const succeededIds = new Set(newValidResults.map(r => r.stockId));
                setFailedStocks(prev => prev.filter(s => !succeededIds.has(s.id)));
                // Also add any that failed again in this batch (though they should already be in prev)
                // Actually, currentlyFailed has the ones that failed in THIS batch.
                // Since batch is from stocksToRetry, these are already in failedStocks.
            } else {
                setFailedStocks(currentlyFailed);
            }

            // Update results
            const combinedResults = stocksToRetry ? [...filterResults, ...newValidResults] : newValidResults;

            if (filterMode === 'distribution') {
                combinedResults.sort((a, b) => (levelOrder[a.distributionLevel] ?? 4) - (levelOrder[b.distributionLevel] ?? 4));
            }
            if (filterMode === 'green') {
                const lightOrder: Record<string, number> = { green: 0, yellow: 1, red: 2 };
                combinedResults.sort((a, b) => (lightOrder[a.data.light] ?? 1) - (lightOrder[b.data.light] ?? 1));
            }

            setFilterProgress({ current: uniqueStocks.length, total: uniqueStocks.length, phase: '分析完成' });
            setFilterResults(combinedResults as any);
            setFilterCompleted(true);
            ...
            <div id="smart-navigator-content" className="container mx-auto px-6 py-12 max-w-2xl min-h-screen">
                ...
                {showAutoFilter && filterCompleted && (
                    <div id="stock-filter-summary-container" className="animate-in fade-in slide-in-from-bottom-8 duration-700">
                        ...
                        {/* Failed Stocks Warning & Retry */}
                        {failedStocks.length > 0 && (
                            <div className="mb-8 p-6 rounded-3xl bg-amber-500/5 border border-amber-500/20 animate-in slide-in-from-top-4 duration-500">
                                <div className="flex items-start gap-4">
                                    <div className="w-12 h-12 rounded-2xl bg-amber-500/10 flex items-center justify-center flex-shrink-0">
                                        <AlertTriangle className="w-6 h-6 text-amber-500" />
                                    </div>
                                    <div className="flex-1">
                                        <h3 className="text-lg font-black text-amber-400 mb-1">部分數據獲取失敗</h3>
                                        <p className="text-slate-400 text-sm leading-relaxed mb-4">
                                            本次篩選中有 {failedStocks.length} 檔股票無法獲取即時市場數據。這可能是由於交易所網路波動或代號異常。
                                        </p>
                                        <button
                                            onClick={() => handleAutoFilter(failedStocks)}
                                            disabled={isFiltering}
                                            className="px-6 py-2.5 bg-amber-500 hover:bg-amber-400 disabled:bg-slate-800 disabled:text-slate-500 text-black font-black rounded-xl text-sm transition-all flex items-center gap-2"
                                        >
                                            {isFiltering ? <Loader2 className="w-4 h-4 animate-spin" /> : <Play className="w-4 h-4 fill-current" />}
                                            針對這些股票再次篩選
                                        </button>
                                    </div>
                                </div>
