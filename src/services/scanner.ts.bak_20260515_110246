...
export const ScannerService = {
    ...
    analyzeStock: async (
        ...
        try {
            ...
            const result = evaluateStock(prices, settings, indexData, isTpex ? 'TPEX' : 'TWSE');
            ...
            // 基礎輔助：投信、融資、營收 (當作額外加分項目，最高加 20分，但不超過100總分)
            const bonus = Math.min((instScore / 30) * 10 + (marginScore / 11) * 5 + (revenueBonusPoints / 13) * 5, 20);
