...
export const ScannerService = {
    ...
    analyzeStock: async (
        ...
        try {
            ...
            // 1. Concurrent Fetching of Stock Data (Prices, Inst, Revenue, Margin)
            const fetchPrices = async (): Promise<StockData[]> => {
                let p: StockData[] = [];
                ...
                if (p.length < 25) {
                    try {
                        ...
                        p = await FinMindClient.getDailyStats({ stockId, startDate, endDate });
                    } catch (e) {
                        console.warn(`[Analyze] FinMind price failed for ${stockId}, fallback to Exchange...`);
                    }
                    if (p.length < 25) {
                        console.warn(`[Analyze] FinMind returned insufficient data (${p.length} days) for ${stockId}, using Exchange fallback...`);
                        warnings.push('Price data fallback to secondary source');
                        try {
                            p = await ExchangeClient.getStockHistory(stockId);
                        } catch (e) {
                            console.error(`[Analyze] Exchange fallback also failed for ${stockId}:`, e);
                        }
                    }
                    if (p.length > 0) {
                        try { await redis.set(priceCacheKey, JSON.stringify(p), 'EX', 14400); } catch (e) { }
                    }
                }
                return p;
            };

            const fetchInsts = async (): Promise<any[]> => {
                let data: any[] = [];
                ...
                if (data.length === 0) {
                    try {
                        data = await FinMindClient.getInstitutional({
                            stockId,
                            startDate: format(subDays(new Date(), 10), 'yyyy-MM-dd'),
                            endDate: todayStr
                        });
                        if (data.length > 0) {
                            try { await redis.set(instCacheKey, JSON.stringify(data), 'EX', 14400); } catch (e) { }
                        }
                    } catch (e) {
            ...
            const fetchRevenue = async (): Promise<any[]> => {
                let data: any[] = [];
                ...
                if (data.length === 0) {
                    try {
                        ...
                        data = await FinMindExtras.getMonthlyRevenue({ stockId, startDate: revStart, endDate: todayStr });
                        if (data.length > 0) {
                            try { await redis.set(revCacheKey, JSON.stringify(data), 'EX', 43200); } catch (e) { }
                        }
                    } catch (e) {
            ...
            const fetchMargin = async (): Promise<any[]> => {
                let data: any[] = [];
                ...
                if (data.length === 0) {
                    try {
                        data = await FinMindExtras.getMarginTrading({
                            stockId,
                            startDate: format(subDays(new Date(), 10), 'yyyy-MM-dd'),
                            endDate: todayStr
                        });
                        if (data.length > 0) {
                            try { await redis.set(marginCacheKey, JSON.stringify(data), 'EX', 14400); } catch (e) { }
                        }
            ...
            // Concurrent execution for 75% faster data loading per stock
            const [pricesRes, instsRes, revRes, marginRes] = await Promise.allSettled([
                fetchPrices(),
                fetchInsts(),
                fetchRevenue(),
                fetchMargin()
            ...
            const marginData = marginRes.status === 'fulfilled' ? marginRes.value : [];
